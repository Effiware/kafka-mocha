...
from kafka_mocha.exceptions import KConsumerTimeoutException, KConsumerMaxRetryException
...
class KConsumer:
    ...
    def commit(self, message: Optional[KMessage] = None, offsets: Optional[list[TopicPartition]] = None, 
    ...
    def _fetch_with_retry(self, request: tuple, timeout: Optional[float] = None) -> list[KMessage]:
        """
        ...
        while count < self._max_retry_count:
            if getgeneratorstate(self._kafka_simulator.consumers_handler) == GEN_SUSPENDED:
                self._kafka_simulator.consumers_handler.send(request)
                messages = next(self._kafka_simulator.consumers_handler)
    ...
    def _consume_messages(self, num_messages: int) -> list[KMessage]:
        ...
        for tp in self._assignment:
            offset = -1
            if (tp.topic in self._positions and 
                tp.partition in self._positions[tp.topic]):
                offset = self._positions[tp.topic][tp.partition]
            topic_partitions.append(TopicPartition(tp.topic, tp.partition, offset))
    ...
    def subscribe(self, topics: list[str], on_assign: Optional[Callable] = None, 
                 on_revoke: Optional[Callable] = None, on_lost: Optional[Callable] = None) -> None:
        """
    ...
    def list_topics(self, topic: Optional[str] = None, timeout: float = -1.0) -> confluent_kafka.admin.ClusterMetadata:
        """
        Request metadata from the cluster.
        
        :param topic: If specified, only request information about this topic.
        :param timeout: The maximum response time before timing out.
    ...
    def committed(self, partitions: list[TopicPartition], timeout: Optional[float] = None) -> list[TopicPartition]:
        """
    ...
    def seek(self, partition: TopicPartition) -> None:
        ...
        if not any(tp.topic == partition.topic and tp.partition == partition.partition 
                  for tp in self._assignment):
            raise confluent_kafka.KafkaException(
                confluent_kafka.KafkaError(
        ...
        if getgeneratorstate(self._kafka_simulator.consumers_handler) == GEN_SUSPENDED:
            ...
            next(self._kafka_simulator.consumers_handler)  # Consume the result
            
        self.logger.debug("Seek to offset %d for %s[%d]", 
                         partition.offset, partition.topic, partition.partition)
        
    def store_offsets(self, message: Optional[KMessage] = None, 
                     offsets: Optional[list[TopicPartition]] = None) -> None:
        """
    ...
    def pause(self, partitions: list[TopicPartition]) -> None:
        ...
        for tp in partitions:
            if not any(p.topic == tp.topic and p.partition == tp.partition 
    ...
    def resume(self, partitions: list[TopicPartition]) -> None:
        ...
        self._paused_partitions = [
            p for p in self._paused_partitions 
    ...
    def get_watermark_offsets(self, partition: TopicPartition, timeout: Optional[float] = None, 
                             cached: bool = False) -> Optional[tuple[int, int]]:
        """
        Retrieve low and high offsets for the specified partition.
        
        :param partition: Topic+partition to return offsets for.
        :param timeout: Request timeout (seconds). Ignored if cached=True.
        :param cached: Instead of querying the broker, use cached information.
